...
async function generateSingleAtlas(data, name) {
    ...
    const json = `${join(assetsPath, 'atlas')}${SLASH}${name}@${options.scale}.${options.textureFormat}.json`;
    const png = `${join(assetsPath, 'atlas')}${SLASH}${name}.png`;
...
async function generateUncompressedSprites() {
    ...
    try {
        ...
        const file = join(assetsPath, `assetsNames${SLASH}assets.ts`);
